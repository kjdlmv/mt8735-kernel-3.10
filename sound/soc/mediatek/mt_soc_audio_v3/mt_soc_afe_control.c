...
bool Register_Aud_Irq(void *dev, uint32 afe_irq_number)
{
    int ret;
#ifdef CONFIG_OF
    ret = request_irq(afe_irq_number, AudDrv_IRQ_handler, IRQF_TRIGGER_LOW , "Afe_ISR_Handle", dev);//IRQF_TRIGGER_LOW
    if (ret)
    {
...
{
	printk(" %s Irqmode = %d Counter = %d ", __func__, Irqmode, SampleRate);
...
bool Set2ndI2SIn(AudioDigtalI2S *mDigitalI2S)
{
    ...
    Audio_I2S_Adc |=	(m2ndI2S->mI2S_HDEN<<12);
