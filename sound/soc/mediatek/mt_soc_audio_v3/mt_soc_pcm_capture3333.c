...
static struct snd_pcm_hardware mtk_capture_hardware =
{
    ...
    .buffer_bytes_max = AWB_MAX_BUFFER_SIZE,//AWB_MAX_BUFFER_SIZE,//UL1_MAX_BUFFER_SIZE,//AWB_MAX_BUFFER_SIZE,//
    .period_bytes_max = AWB_MAX_BUFFER_SIZE,//AWB_MAX_BUFFER_SIZE,//AWB_MAX_BUFFER_SIZE,//UL1_MAX_BUFFER_SIZE,
    .periods_min =  AWB_MIN_PERIOD_SIZE,   //UL1_MIN_PERIOD_SIZE,
    .periods_max =  AWB_MAX_PERIOD_SIZE,// AWB_MAX_PERIOD_SIZE,   // UL1_MAX_PERIOD_SIZE,
    .fifo_size =      0,
};
...
static void StartAudioCaptureHardware(struct snd_pcm_substream *substream)
{
 ...
 if(substream->runtime->rate == Samp_96)
	{	
		 mt_set_gpio_mode(GPIO_I2S0_CK_PIN, GPIO_MODE_01); //GPIO24, AUD_CLK_MOSI	      
	            mt_set_gpio_mode(GPIO_I2S0_DAT_PIN, GPIO_MODE_01); //GPIO25, AUD_DAT_MISO	   
	             mt_set_gpio_mode(GPIO_I2S0_WS_PIN, GPIO_MODE_01); //GPIO26, AUD_DAT_MOSI
		...
		mt_set_gpio_pull_enable(GPIO_I2S0_CK_PIN, GPIO_PULL_DISABLE);
		mt_set_gpio_pull_enable(GPIO_I2S0_DAT_PIN, GPIO_PULL_DISABLE);
		mt_set_gpio_pull_enable(GPIO_I2S0_WS_PIN, GPIO_PULL_DISABLE);
 }
	EnableAfe(true);
...
static void MY_SetAWBBuffer(struct snd_pcm_substream *substream)
{
...
static void MY_SetVULBuffer(struct snd_pcm_substream *substream)                   
{
    struct snd_pcm_runtime *runtime = substream->runtime;
    AFE_BLOCK_T *pblock = &VUL_Control_context->rBlock;
    printk("SetVULBuffer\n");
...
static struct snd_pcm_hw_params *my_hw_params;
...
static int mtk_capture_pcm_prepare(struct snd_pcm_substream *substream)
{
   ...
   if(substream->runtime->rate == Samp_96) 
   {
   	Capture_dma_buf = AWBCapture_dma_buf; 
	VUL_Control_context = Get_Mem_ControlT(Soc_Aud_Digital_Block_MEM_AWB);
   }
   else  							    
   {
   	Capture_dma_buf =VULCapture_dma_buf;
    ...
    {
        runtime->dma_bytes = params_buffer_bytes(my_hw_params);      
    ...
    else
    {
        ...
        ret =  snd_pcm_lib_malloc_pages(substream, params_buffer_bytes(my_hw_params));
    }

	if(substream->runtime->rate == Samp_96) MY_SetAWBBuffer(substream);
	else								 MY_SetVULBuffer(substream);
#endif
	return 0;
}
...
{
    printk("mtk_capture_alsa_start rate=%d \n",substream->runtime->rate);
   if(substream->runtime->rate == Samp_96)	
...
static int mtk_capture_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	switch (cmd)
	{
		...
		if(substream->stream ==SNDRV_PCM_STREAM_CAPTURE)
		{
			...
			{
				fpga_init_gpio();
				mt_set_gpio_mode(GPIO_TDI_RST_PIN, GPIO_MODE_00);  
				mt_set_gpio_dir(GPIO_TDI_RST_PIN, GPIO_DIR_OUT) ; 
				mt_set_gpio_out(GPIO_TDI_RST_PIN,1);
			}
		}
		...
		if(substream->stream ==SNDRV_PCM_STREAM_CAPTURE)
		{
			if(substream->runtime->rate == Samp_96)
			{
			  mt_set_gpio_out(GPIO_TDI_RST_PIN,0);
			}
		}
			return ret;
	}
...
static int mtk_afe_capture_probe(struct snd_soc_platform *platform)
{
    ...
    AudDrv_Allocate_mem_Buffer(platform->dev, Soc_Aud_Digital_Block_MEM_AWB, AWB_MAX_BUFFER_SIZE);//AWB_MAX_BUFFER_SIZE
    AWBCapture_dma_buf =  Get_Mem_Buffer(Soc_Aud_Digital_Block_MEM_AWB);

    AudDrv_Allocate_mem_Buffer(platform->dev, Soc_Aud_Digital_Block_MEM_VUL, UL1_MAX_BUFFER_SIZE);//UL1_MAX_BUFFER_SIZE
    VULCapture_dma_buf =  Get_Mem_Buffer(Soc_Aud_Digital_Block_MEM_VUL);
