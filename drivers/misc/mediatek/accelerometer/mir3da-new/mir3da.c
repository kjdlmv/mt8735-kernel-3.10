...
static int mir3da_readCalibration(struct i2c_client *client, int dat[MIR3DA_AXES_NUM])
{
    ...
    MI_MSG("mir3da_readCalibration:%d-%d-%d\n",dat[obj->cvt.map[MIR3DA_AXIS_X]],dat[obj->cvt.map[MIR3DA_AXIS_Y]],dat[obj->cvt.map[MIR3DA_AXIS_Z]]);                                   
    return 0;
}
/*----------------------------------------------------------------------------*/
static int mir3da_writeCalibration(struct i2c_client *client, int dat[MIR3DA_AXES_NUM])
{
    ...
    {
        MI_MSG("obj->cali_sw[xyz=]=%d.%d.%d\n",obj->cali_sw[MIR3DA_AXIS_X],obj->cali_sw[MIR3DA_AXIS_Y],obj->cali_sw[MIR3DA_AXIS_Z]);
...
{    
    ...
    else
    {
      ...
      if(bzstk)
			   acc[MIR3DA_AXIS_Z] =obj->cvt.sign[MIR3DA_AXIS_Z]*squareRoot(1024*1024 - acc[MIR3DA_AXIS_X]*acc[MIR3DA_AXIS_X] - acc[MIR3DA_AXIS_Y]*acc[MIR3DA_AXIS_Y]); 	
#endif
...
#ifdef CONFIG_COMPAT
static long mir3da_compat_ioctl(struct file *file, unsigned int cmd,
       unsigned long arg)
{
    long err = 0;
    ...
    switch (cmd)
    {
        case COMPAT_GSENSOR_IOCTL_READ_SENSORDATA:
		    ...
		    MI_MSG("IOCTRL --- COMPAT_GSENSOR_IOCTL_READ_SENSORDATA\n");
        ...
        case COMPAT_GSENSOR_IOCTL_INIT:
		 ...
		 MI_MSG("IOCTRL --- COMPAT_GSENSOR_IOCTL_INIT\n");
		    err = file->f_op->unlocked_ioctl(file, GSENSOR_IOCTL_INIT, (unsigned long )arg64);
		    if (err){
		        MI_ERR("GSENSOR_IOCTL_INIT unlocked_ioctl failed.");
		        return err;
		    }
        break;
		...
		MI_MSG("IOCTRL --- COMPAT_GSENSOR_IOCTL_READ_CHIPINFO\n");
		    err = file->f_op->unlocked_ioctl(file, GSENSOR_IOCTL_READ_CHIPINFO, (unsigned long )arg64);
		    if (err){
		        MI_ERR("COMPAT_GSENSOR_IOCTL_READ_CHIPINFO unlocked_ioctl failed.");
		        return err;
		    }
        break;
		...
		MI_MSG("IOCTRL --- COMPAT_GSENSOR_IOCTL_READ_GAIN\n");
		    err = file->f_op->unlocked_ioctl(file, GSENSOR_IOCTL_READ_GAIN, (unsigned long )arg64);
		    if (err){
		        MI_ERR("COMPAT_GSENSOR_IOCTL_READ_GAIN unlocked_ioctl failed.");
		        return err;
		    }
        break;
		...
		MI_MSG("IOCTRL --- COMPAT_GSENSOR_IOCTL_READ_OFFSET\n");
		    err = file->f_op->unlocked_ioctl(file, GSENSOR_IOCTL_READ_OFFSET, (unsigned long )arg64);
		    if (err){
		        MI_ERR("COMPAT_GSENSOR_IOCTL_READ_OFFSET unlocked_ioctl failed.");
		        return err;
		    }
        break;
		...
		MI_MSG("IOCTRL --- COMPAT_GSENSOR_IOCTL_READ_RAW_DATA\n");
		    err = file->f_op->unlocked_ioctl(file, GSENSOR_IOCTL_READ_RAW_DATA, (unsigned long )arg64);
		    if (err){
		        MI_ERR(" COMPAT_GSENSOR_IOCTL_READ_RAW_DATA failed.");
		        return err;
		    }
        break;
		...
		MI_MSG("IOCTRL --- COMPAT_GSENSOR_IOCTL_SET_CALI\n");
		    err = file->f_op->unlocked_ioctl(file, GSENSOR_IOCTL_SET_CALI, (unsigned long )arg64);
		    if (err){
		        MI_ERR("COMPAT_GSENSOR_IOCTL_SET_CALI unlocked_ioctl failed.");
		        return err;
		    }
        break;
		...
		MI_MSG("IOCTRL --- COMPAT_GSENSOR_IOCTL_CLR_CALI\n");
		    err = file->f_op->unlocked_ioctl(file, GSENSOR_IOCTL_CLR_CALI, (unsigned long )arg64);
		    if (err){
		        MI_ERR("COMPAT_GSENSOR_IOCTL_CLR_CALI unlocked_ioctl failed.");
		        return err;
		    }
        break;
		...
		MI_MSG("IOCTRL --- COMPAT_GSENSOR_IOCTL_GET_CALI\n");
		    err = file->f_op->unlocked_ioctl(file, GSENSOR_IOCTL_GET_CALI, (unsigned long )arg64);
		    if (err){
		        MI_ERR("COMPAT_GSENSOR_IOCTL_GET_CALI unlocked_ioctl failed.");
		        return err;
		    }
        break;
...
static long mir3da_misc_ioctl( struct file *file,unsigned int cmd, unsigned long arg)
{
    ...
    case GSENSOR_IOCTL_GET_CALI:
        ...
        {
            MI_MSG("IOCTRL --- GSENSOR_IOCTL_GET_CALI 0\n");
            break;
        }
        MI_MSG("IOCTRL --- GSENSOR_IOCTL_GET_CALI 1\n");
        ...
        {
            MI_ERR("IOCTRL --- GSENSOR_IOCTL_GET_CALI,copy to user space failed\n");
            err = -EFAULT;
            break;
        }        
        break;  
...
static const struct file_operations mir3da_misc_fops = {
	...
	#ifdef CONFIG_COMPAT
	.compat_ioctl = mir3da_compat_ioctl,
	#endif
};
...
#if MIR3DA_OFFSET_TEMP_SOLUTION
static DRIVER_ATTR(primary_offset,  S_IWUGO | S_IRUGO,  mir3da_primary_offset_show,     NULL);static DRIVER_ATTR(offset,          S_IWUGO | S_IRUGO,  mir3da_offset_show,             mir3da_offset_store);
...
static int mir3da_enable_nodata(int en)
{
	...
	res = mir3da_setPowerMode(obj_i2c_data->client, power);
	if(res != MIR3DA_SUCCESS)
	{
		MI_ERR("mir3da_SetPowerMode fail!\n");
		return -1;
	}
	MI_ERR("mir3da_enable_nodata OK!\n");
	return 0;
}
...
static int mir3da_set_delay(u64 ns)
{
	...
	if(value <= 5)
	{
		sample_delay = mir3da_BW_200HZ;
	}
	else if(value <= 10)
	{
		sample_delay = mir3da_BW_100HZ;
	}
	else
	{
		sample_delay = mir3da_BW_50HZ;
	}
	mutex_lock(&mir3da_op_mutex);
	err = mir3da_SetBWRate(obj_i2c_data->client, sample_delay);
	if(err != mir3da_SUCCESS ) //0x2C->BW=100Hz
	{
		MI_ERR("Set delay parameter error!\n");
	}
	mutex_unlock(&mir3da_op_mutex);
	if(value >= 50)
	{
	...
	{					
		...
		obj_i2c_data->fir.sum[mir3da_AXIS_X] = 0;
		obj_i2c_data->fir.sum[mir3da_AXIS_Y] = 0;
		obj_i2c_data->fir.sum[mir3da_AXIS_Z] = 0;
		atomic_set(&obj_i2c_data->filter, 1);
	}
	
	MI_ERR("mir3da_set_delay (%d)\n",value);
	#endif
	return 0;
}
...
static int /* __devinit */ mir3da_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
  ...
  ctl.open_report_data= mir3da_open_report_data;
	...
	if(result)
	{
	 	MI_ERR("register acc data path err\n");
		goto exit_kfree;
	}

	result = batch_register_support_info(ID_ACCELEROMETER,ctl.is_support_batch, 1000, 0);
    if(result)
    {
        MI_ERR("register gsensor batch support err = %d\n", result);
        goto exit_create_attr_failed;
    }
