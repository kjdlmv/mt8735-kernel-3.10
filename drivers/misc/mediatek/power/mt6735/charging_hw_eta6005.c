...
static kal_uint32 charging_hw_init(void *data)
{
   ...
   mt_set_gpio_mode(GPIO_USB_DEFAULT_DEVICE_MODE,GPIO_MODE_00);  // gpio mode
   mt_set_gpio_pull_enable(GPIO_USB_DEFAULT_DEVICE_MODE,GPIO_PULL_ENABLE);
   mt_set_gpio_dir(GPIO_USB_DEFAULT_DEVICE_MODE,GPIO_DIR_OUT); // output 
   mt_set_gpio_out(GPIO_USB_DEFAULT_DEVICE_MODE,GPIO_OUT_ONE); //default select ISET2,500mA

	mt_set_gpio_mode((GPIO68 | 0x80000000),GPIO_MODE_00);  
	mt_set_gpio_dir((GPIO68 | 0x80000000),GPIO_DIR_IN);
...
static kal_uint32 charging_get_current(void *data)
{
	...
	kal_uint32 val = 0;
	//TODO
    out = mt_get_gpio_out(GPIO_USB_DEFAULT_DEVICE_MODE);
    if (out == 0)
        val = 500;
    else if (out == 1)
        val = 2000;
...
//add-wujie@wisky20151226
#if defined(CONFIG_WISKY_4G005K)
    if(g_is_lcd_suspend == 0)
    {
	    	mt_set_gpio_out(GPIO_USB_DEFAULT_DEVICE_MODE,GPIO_OUT_ONE); //select ISET2,500mA
	  }
	  else
	  {
	  	  if ( current_value <= USB_CHARGER_CURRENT)
        	mt_set_gpio_out(GPIO_USB_DEFAULT_DEVICE_MODE,GPIO_OUT_ONE); //select ISET2,500mA
    		else 
        	mt_set_gpio_out(GPIO_USB_DEFAULT_DEVICE_MODE,GPIO_OUT_ZERO); //select ISET1,2000mA
	  }
#else
    if ( current_value <= USB_CHARGER_CURRENT)
        mt_set_gpio_out(GPIO_USB_DEFAULT_DEVICE_MODE,GPIO_OUT_ONE); //select ISET2,500mA
...
#endif
    //end
    pr_notice("--->charging_set_current=%d\n",current_value);
	return status;
} 	
...
static kal_uint32 charging_set_input_current(void *data)
{
    ...
    pr_notice("--->charging_set_input_current=%d\n",current_value);
...
static kal_uint32 charging_get_charging_status(void *data)
{
	kal_uint32 status = STATUS_OK;
	kal_uint32 ret_val;
    int stat_pin_level = -1;
    
    stat_pin_level = mt_get_gpio_out((GPIO68 | 0x80000000));

    if (stat_pin_level == 1)
	    *(kal_uint32 *)data = KAL_TRUE;
		else
	    *(kal_uint32 *)data = KAL_FALSE;
    pr_notice("--->charging_get_charging_status,stat_pin_level=%d\n",stat_pin_level);
...
static kal_uint32 charging_get_charger_det_status(void *data)
{
	...
	kal_uint32 val = 0;
		...
		val = 1;
		battery_xlog_printk(BAT_LOG_CRTI,"[charging_get_charger_det_status] chr exist for fpga.\n"); 
	#else    
...
static kal_uint32 charging_set_ta_current_pattern(void *data)
{
    ...
    pr_notice("--->charging_set_ta_current_pattern,TODO,need CONFIG_MTK_PUMP_EXPRESS_PLUS_SUPPORT!!!\n");
