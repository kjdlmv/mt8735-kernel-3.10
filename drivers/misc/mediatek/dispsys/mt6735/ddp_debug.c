...
static void process_dbg_opt(const char *opt)
{
	...
	else if(0 == strncmp(opt, "rmmsys", 6)) {
		sprintf(buf, "rmmsys: %x\n", INREG32(DISP_REG_CONFIG_MMSYS_CG_CON1));   
	}
	else if(0 == strncmp(opt, "wmmsys:", 7)) {

		sscanf(opt+7, "%x", &val);
		printk("wmmsys:0x%08x\n",val);
		OUTREG32(DISP_REG_CONFIG_MMSYS_CG_CON1 ,val);
		sprintf(buf, "wmmsys: %x\n", INREG32(DISP_REG_CONFIG_MMSYS_CG_CON1));   

	}
	else if(0 == strncmp(opt, "colorbaron", 10)) {
		OUTREG32(DISPSYS_DPI_BASE + 0XF00,0X41);
		sprintf(buf, "colorbaron\n");     

	}
	else if(0 == strncmp(opt, "colorbaroff", 11)) {
			OUTREG32(DISPSYS_DPI_BASE + 0XF00,0x00);
			sprintf(buf, "colorbaroff\n");	   
	}
	else if(0 == strncmp(opt, "dpidump", 7)) {
		sprintf(buf, "dpidump\n");  
			printk("---------- Start dump DPI registers ----------\n");
		
			for (ii = 0; ii <= 0x54; ii += 4)
			{
				printk("DPI+%04x : 0x%08x\n", ii, INREG32(DISPSYS_DPI_BASE + ii));
			}	
			for (ii = 0x68; ii <= 0xd8; ii += 4)
			{
				printk("DPI+%04x : 0x%08x\n", ii, INREG32(DISPSYS_DPI_BASE + ii));
			}
		
			printk("DPI+Color Bar : %04x : 0x%08x\n", 0xF00, INREG32(DISPSYS_DPI_BASE + 0xF00));
		//	  DDPDUMP("DPI Addr IO Driving : 0x%08x\n", INREG32(DISPSYS_IO_DRIVING));
		
			printk("DPI TVDPLL CON0 : 0x%08x\n",  INREG32(DDP_REG_TVDPLL_CON0));
			printk("DPI TVDPLL CON1 : 0x%08x\n",  INREG32(DDP_REG_TVDPLL_CON1));
			printk("DPI TVDPLL CON6 : 0x%08x\n",  INREG32(DDP_REG_TVDPLL_CON6));
		
			printk("DPI MMSYS_CG_CON1:0x%08x\n",  INREG32(DISP_REG_CONFIG_MMSYS_CG_CON1));
			printk("io_driving1:0x:%08x\n",INREG32(DISPSYS_IO_DRIVING1));
			printk("io_driving2:0x:%08x\n",INREG32(DISPSYS_IO_DRIVING2));
