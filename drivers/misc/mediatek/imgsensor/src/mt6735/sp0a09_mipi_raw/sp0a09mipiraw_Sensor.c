...
static imgsensor_info_struct imgsensor_info = { 
    ...
    .pre = {
        ...
        .linelength = 806,             //record different mode's linelength
        .framelength = 499,            //record different mode's framelength
        .startx = 0,                    //record different mode's startx of grabwindow
        .starty = 0,                    //record different mode's starty of grabwindow
		.grabwindow_width = 640,//2096,		//record different mode's width of grabwindow
		.grabwindow_height =480,//1552,		//record different mode's height of grabwindow
...
{ {  648, 488,  4, 4, 640, 480, 640, 480, 0, 0, 640, 480,  0, 0, 640, 480}, // Preview 2112*1558                          
  ...
  {  648, 488,  4, 4, 640, 480, 640, 480, 0, 0, 640, 480,  0, 0, 640, 480}, // Custom1 (defaultuse preview)               
  {  648, 488,  4, 4, 640, 480, 640, 480, 0, 0, 640, 480,  0, 0, 640, 480}, // Custom2                                    
  {  648, 488,  4, 4, 640, 480, 640, 480, 0, 0, 640, 480,  0, 0, 640, 480}, // Custom3                                    
  {  648, 488,  4, 4, 640, 480, 640, 480, 0, 0, 640, 480,  0, 0, 640, 480}, // Custom4                                    
  {  648, 488,  4, 4, 640, 480, 640, 480, 0, 0, 640, 480,  0, 0, 640, 480}, // Custom5                                    
 };// slim video  
...
/* Sensor output window information *//*
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] =    
{ {  640, 480,  8, 8, 624, 464, 624, 464, 0, 0, 624, 464,  0, 0, 624, 464}, // Preview 2112*1558
  {  640, 480,  8, 8, 624, 464, 624, 464, 0, 0, 624, 464,  0, 0, 624, 464}, // capture 4206*3128
  {  640, 480,  8, 8, 624, 464, 624, 464, 0, 0, 624, 464,  0, 0, 624, 464}, // video 
  {  640, 480,  8, 8, 624, 464, 624, 464, 0, 0, 624, 464,  0, 0, 624, 464}, //hight speed video 
  {  640, 480,  8, 8, 624, 464, 624, 464, 0, 0, 624, 464,  0, 0, 624, 464},// slim video
  {  640, 480,  8, 8, 624, 464, 624, 464, 0, 0, 624, 464,  0, 0, 624, 464}, // Custom1 (defaultuse preview) 
  {  640, 480,  8, 8, 624, 464, 624, 464, 0, 0, 624, 464,  0, 0, 624, 464}, // Custom2 
  {  640, 480,  8, 8, 624, 464, 624, 464, 0, 0, 624, 464,  0, 0, 624, 464}, // Custom3 
  {  640, 480,  8, 8, 624, 464, 624, 464, 0, 0, 624, 464,  0, 0, 624, 464}, // Custom4 
  {  640, 480,  8, 8, 624, 464, 624, 464, 0, 0, 624, 464,  0, 0, 624, 464}, // Custom5 
 };// slim video  
 */
...
//#define DEBUG_SENSOR
#ifdef DEBUG_SENSOR
	#define SP0A09MIPI_OP_CODE_INI		0x00		/* Initial value. */
	#define SP0A09MIPI_OP_CODE_REG		0x01		/* Register */
	#define SP0A09MIPI_OP_CODE_DLY		0x02		/* Delay */
	#define SP0A09MIPI_OP_CODE_END		0x03		/* End of initial setting. */
	...
	} SP0A09MIPI_initial_set_struct;

	SP0A09MIPI_initial_set_struct SP0A09MIPI_Init_Reg[1000];
 ...
 u32 strtol(const char *nptr, u8 base)
{
 ...
 u8 SP0A09MIPI_Initialize_from_T_Flash()
{
	...
	fp = filp_open("/mnt/sdcard/sp0a09_sd", O_RDONLY , 0); 
	if (IS_ERR(fp)) 
	{ 
	...
	while (curr_ptr < (data_buff + file_size))
	{
		...
		if (strcmp((const char *)func_ind, "REG") == 0)		/* REG */
		{
			...
			SP0A09MIPI_Init_Reg[i].op_code = SP0A09MIPI_OP_CODE_REG;
			
			SP0A09MIPI_Init_Reg[i].init_reg = strtol((const char *)curr_ptr, 16);
			curr_ptr += 5;	/* Skip "00, 0x" */
		
			SP0A09MIPI_Init_Reg[i].init_val = strtol((const char *)curr_ptr, 16);
			curr_ptr += 4;	/* Skip "00);" */
		...
		else									/* DLY */
		{
			...
			curr_ptr += 4;	
			SP0A09MIPI_Init_Reg[i].op_code = SP0A09MIPI_OP_CODE_DLY;
			
			SP0A09MIPI_Init_Reg[i].init_reg = 0xFF;
			SP0A09MIPI_Init_Reg[i].init_val = strtol((const char *)curr_ptr,  10);	/* Get the delay ticks, the delay should less then 50 */
		}
		i++;
	...
	SP0A09MIPI_Init_Reg[i].op_code = SP0A09MIPI_OP_CODE_END;
	SP0A09MIPI_Init_Reg[i].init_reg = 0xFF;
	SP0A09MIPI_Init_Reg[i].init_val = 0xFF;
	i++;
	//for (j=0; j<i; j++)
		//printk(" %x  ==  %x\n",SP0A09MIPI_Init_Reg[j].init_reg, SP0A09MIPI_Init_Reg[j].init_val);

	/* Start apply the initial setting to sensor. */
	#if 1
	for (j=0; j<i; j++)
	{
		if (SP0A09MIPI_Init_Reg[j].op_code == SP0A09MIPI_OP_CODE_END)	/* End of the setting. */
		{
			break ;
		}
		else if (SP0A09MIPI_Init_Reg[j].op_code == SP0A09MIPI_OP_CODE_DLY)
		{
			msleep(SP0A09MIPI_Init_Reg[j].init_val);		/* Delay */
		}
		else if (SP0A09MIPI_Init_Reg[j].op_code == SP0A09MIPI_OP_CODE_REG)
		{
		
			write_cmos_sensor(SP0A09MIPI_Init_Reg[j].init_reg, SP0A09MIPI_Init_Reg[j].init_val);
			printk("%x = %x\n",SP0A09MIPI_Init_Reg[j].init_reg,SP0A09MIPI_Init_Reg[j].init_val);
		}
		else
		{
...
static void sensor_init(void)
{
	...
	//new para __  zhangliang

	    #ifdef DEBUG_SENSOR
	if(fromsd == 1)//鏄惁浠嶴D璇诲彇//gepeiwei   120903
		SP0A09MIPI_Initialize_from_T_Flash();//浠嶴D鍗¤鍙栫殑涓昏鍑芥暟
       else
    #endif
  ...
  write_cmos_sensor(0x16,0x20);//2c
  write_cmos_sensor(0x17,0x20);//2d
  write_cmos_sensor(0x70,0x22);//2e
  write_cmos_sensor(0x6a,0x28);//18
  write_cmos_sensor(0x72,0x2a);//1a  
  write_cmos_sensor(0x1a,0x0b);//09
  write_cmos_sensor(0x1b,0x03);
  write_cmos_sensor(0x1e,0x13);
  write_cmos_sensor(0x1f,0x01);
  write_cmos_sensor(0x27,0x9b);
  write_cmos_sensor(0x28,0x4f);//4f
  write_cmos_sensor(0x21,0x0c);//0b
  ...
  write_cmos_sensor(0xfd,0x00);  
 	 ...
 	 LOG_INF("Exit sen0xa4=0x01sor_init.\n");  
}   /*  sensor_init  */
...
static kal_uint32 open(void)
{
    ...
    if (imgsensor_info.sensor_id != sensor_id)
      ...
      struct file *fp; 
	    ...
	    {
		    fromsd = 1;
	          //SP0A09MIPI_Initialize_from_T_Flash();
	          printk("gpw read ok!\n");
