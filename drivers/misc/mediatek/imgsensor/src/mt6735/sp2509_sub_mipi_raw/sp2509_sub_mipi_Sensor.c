...
#include "sp2509_sub_mipi_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "SP2509_sub_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = SP2509_SUB_MIPI_SENSOR_ID,
    ...
    .sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_R,//sensor output first pixel color  //SENSOR_OUTPUT_FORMAT_RAW_B
...
#define DEBUG_SENSOR
	...
	sp2509MIPI_initial_set_struct sp2509_Sub_MIPI_Init_Reg[1000];
 ...
 static u32 strtol(const char *nptr, u8 base)
{
 ...
 u8 sp2509_Sub_MIPI_Initialize_from_T_Flash()
{
	...
	while (curr_ptr < (data_buff + file_size))
	{
		...
		if (strcmp((const char *)func_ind, "REG") == 0)		/* REG */
		{
			...
			sp2509_Sub_MIPI_Init_Reg[i].op_code = sp2509MIPI_OP_CODE_REG;
			
			sp2509_Sub_MIPI_Init_Reg[i].init_reg = strtol((const char *)curr_ptr, 16);
			curr_ptr += 5;	/* Skip "00, 0x" */
		
			sp2509_Sub_MIPI_Init_Reg[i].init_val = strtol((const char *)curr_ptr, 16);
			curr_ptr += 4;	/* Skip "00);" */
		...
		else 									/* DLY */
		{
			...
			curr_ptr += 4;	
			sp2509_Sub_MIPI_Init_Reg[i].op_code = sp2509MIPI_OP_CODE_DLY;
			
			sp2509_Sub_MIPI_Init_Reg[i].init_reg = 0xFF;
			sp2509_Sub_MIPI_Init_Reg[i].init_val = strtol((const char *)curr_ptr,  10);	/* Get the delay ticks, the delay should less then 50 */
		}
		i++;
	...
	sp2509_Sub_MIPI_Init_Reg[i].op_code = sp2509MIPI_OP_CODE_END;
	sp2509_Sub_MIPI_Init_Reg[i].init_reg = 0xFF;
	sp2509_Sub_MIPI_Init_Reg[i].init_val = 0xFF;
	i++;
	//for (j=0; j<i; j++)
		//printk(" %x  ==  %x\n",sp2509_Sub_MIPI_Init_Reg[j].init_reg, sp2509_Sub_MIPI_Init_Reg[j].init_val);

	/* Start apply the initial setting to sensor. */
	#if 1
	for (j=0; j<i; j++)
	{
		if (sp2509_Sub_MIPI_Init_Reg[j].op_code == sp2509MIPI_OP_CODE_END)	/* End of the setting. */
		{
			break ;
		}
		else if (sp2509_Sub_MIPI_Init_Reg[j].op_code == sp2509MIPI_OP_CODE_DLY)
		{
			msleep(sp2509_Sub_MIPI_Init_Reg[j].init_val);		/* Delay */
		}
		else if (sp2509_Sub_MIPI_Init_Reg[j].op_code == sp2509MIPI_OP_CODE_REG)
		{
		
			write_cmos_sensor(sp2509_Sub_MIPI_Init_Reg[j].init_reg, sp2509_Sub_MIPI_Init_Reg[j].init_val);
			printk("%x = %x\n",sp2509_Sub_MIPI_Init_Reg[j].init_reg,sp2509_Sub_MIPI_Init_Reg[j].init_val);
		}
		else
		{
...
static void sensor_init(void)
{
	...
	if(fromsd == 1)//是否从SD读取//gepeiwei   120903
		...
		sp2509_Sub_MIPI_Initialize_from_T_Flash();//从SD卡读取的主要函数
