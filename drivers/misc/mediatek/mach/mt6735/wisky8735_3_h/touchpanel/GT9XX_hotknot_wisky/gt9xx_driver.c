...
TP_TYPE_T gtp_touch_ic = TP_TYPE_GT9157;		//gt912
...
int gtp_sensor_id = 0;
u8 gtp_opr_buf[10] = {0x00};
char ic_model[10] = {"GT"};
int gtp_silde_wake_off = 1;
...
static int gt91xx_config_read_proc(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
    ...
    ptr += sprintf(ptr, "status:%d\n", (tpd_halt+1)&0x1);
		ptr += sprintf(ptr, "sensor ID:%d\n",gtp_sensor_id);
		ptr += sprintf(ptr, "gtp_chip_type:%d\n",gtp_chip_type);
		ptr += sprintf(ptr, "gtp_opr_buf:%s\n",gtp_opr_buf);
		ptr += sprintf(ptr, "ic_model:%s\n",ic_model);
...
static int gt91xx_config_write_proc(struct file *file, const char *buffer, size_t count, loff_t *ppos)
{
    ...
    if(strcmp(mode_str, "silde_wake_off") == 0)
    {
        GTP_INFO("silde_wake_off");
        gtp_silde_wake_off = 1;
        return count;              	
    }
    if(strcmp(mode_str, "silde_wake_on") == 0)
    {
        GTP_INFO("silde_wake_on");
        gtp_silde_wake_off = 0;
        return count;              	
    }
...
Output:
    Executive outcomes.0---succeed.
*******************************************************/
//wisky-taowei@20140818
//GT911
const u8 cfg_info_group1[] = CTP_CFG_GROUP1;
const u8 cfg_info_group2[] = CTP_CFG_GROUP2;
const u8 cfg_info_group3[] = CTP_CFG_GROUP3;
const u8 cfg_info_group4[] = CTP_CFG_GROUP4;
const u8 cfg_info_group5[] = CTP_CFG_GROUP5;
const u8 cfg_info_group6[] = CTP_CFG_GROUP6;
//GT928
const u8 cfg_info_group7[] = CTP_CFG_GROUP7;
const u8 cfg_info_group8[] = CTP_CFG_GROUP8;
const u8 cfg_info_group9[] = CTP_CFG_GROUP9;
const u8 cfg_info_group10[] = CTP_CFG_GROUP10;
const u8 cfg_info_group11[] = CTP_CFG_GROUP11;
const u8 cfg_info_group12[] = CTP_CFG_GROUP12;    
//GT9271
const u8 cfg_info_group13[] = CTP_CFG_GROUP13;
const u8 cfg_info_group14[] = CTP_CFG_GROUP14;
const u8 cfg_info_group15[] = CTP_CFG_GROUP15;
const u8 cfg_info_group16[] = CTP_CFG_GROUP16;
const u8 cfg_info_group17[] = CTP_CFG_GROUP17;
const u8 cfg_info_group18[] = CTP_CFG_GROUP18;  
//GT912
const u8 cfg_info_group19[] = CTP_CFG_GROUP19;
const u8 cfg_info_group20[] = CTP_CFG_GROUP20;
const u8 cfg_info_group21[] = CTP_CFG_GROUP21;
const u8 cfg_info_group22[] = CTP_CFG_GROUP22;
const u8 cfg_info_group23[] = CTP_CFG_GROUP23;
const u8 cfg_info_group24[] = CTP_CFG_GROUP24;    
//GT9110
const u8 cfg_info_group25[] = CTP_CFG_GROUP25;
const u8 cfg_info_group26[] = CTP_CFG_GROUP26;
const u8 cfg_info_group27[] = CTP_CFG_GROUP27;
const u8 cfg_info_group28[] = CTP_CFG_GROUP28;
const u8 cfg_info_group29[] = CTP_CFG_GROUP29;
const u8 cfg_info_group30[] = CTP_CFG_GROUP30;  
//end-taowei
static s32 gtp_init_panel(struct i2c_client *client)
{
	...
	u8 *send_cfg_buf[] = {(u8*)cfg_info_group1, (u8*)cfg_info_group2, (u8*)cfg_info_group3,
                        (u8*)cfg_info_group4, (u8*)cfg_info_group5, (u8*)cfg_info_group6,
                        (u8*)cfg_info_group7, (u8*)cfg_info_group8, (u8*)cfg_info_group9,
                        (u8*)cfg_info_group10, (u8*)cfg_info_group11, (u8*)cfg_info_group12,
                        (u8*)cfg_info_group13, (u8*)cfg_info_group14, (u8*)cfg_info_group15,
                        (u8*)cfg_info_group16, (u8*)cfg_info_group17, (u8*)cfg_info_group18,
                        (u8*)cfg_info_group19, (u8*)cfg_info_group20, (u8*)cfg_info_group21,
                        (u8*)cfg_info_group22, (u8*)cfg_info_group23, (u8*)cfg_info_group24,
                        (u8*)cfg_info_group25, (u8*)cfg_info_group26, (u8*)cfg_info_group27,
                        (u8*)cfg_info_group28, (u8*)cfg_info_group29, (u8*)cfg_info_group30};
	...
	printk("[GTP info>>> >>>] detected ic model :%s\n", ic_model);
    if (strcmp(ic_model, "GT911") == 0){
		sensor_id += 0;
		gtp_touch_ic = TP_TYPE_GT911;
	}else if (strcmp(ic_model, "GT928") == 0){
		sensor_id += 6;
		gtp_touch_ic = TP_TYPE_GT928;
	}else if (strcmp(ic_model, "GT9271") == 0){
		sensor_id += 12;
		gtp_touch_ic = TP_TYPE_GT9271;
	}else if (strcmp(ic_model, "GT9157") == 0){
		sensor_id += 18;
		gtp_touch_ic = TP_TYPE_GT9157;
	}else if (strcmp(ic_model, "GT9110") == 0){		
		sensor_id += 24;
		gtp_touch_ic = TP_TYPE_GT9110;
	}else{
		sensor_id += 18;//default gt912,qiutian
		gtp_touch_ic = TP_TYPE_GT9157;
	}
...
void gtp_get_chip_type(struct i2c_client *client)
{
    ...
    if (FAIL == ret)
    {
        ...
        memcpy(gtp_opr_buf,"error",10);
        return;
    }
    memcpy(gtp_opr_buf,opr_buf,10);
...
//#ifdef VELOCITY_CUSTOM
    ...
    gt91xx_config_proc = proc_create(GT91XX_CONFIG_PROC_FILE, 0666, NULL, &gt_upgrade_proc_fops); //wlt@wisky 0660 -> 0666
   ...
   u8  rqst_data[3] = {(u8)(GTP_REG_RQST >> 8), (u8)(GTP_REG_RQST & 0xFF), 0};
    ...
    do
    {
        ...
        if (DOZE_ENABLED == doze_status)
        {
            ...
            {     
                if ((doze_buf[2] == 'c') || (doze_buf[2] == 'e') || (doze_buf[2] == 'm') || 
					(doze_buf[2] == 'o') || (doze_buf[2] == 'w') || (doze_buf[2] == 'Z') ||
					(doze_buf[2] == 'V') || (doze_buf[2] == 'S') ||	(doze_buf[2] == '>') ||
					(doze_buf[2] == 0x5E) /* ^ */
                    )
                {
                    if(doze_buf[2] == 'c'){
											GTP_DEBUG("sile wake C");
		...
		*/
        ...
        GTP_DEBUG("ceshi:ic_model = %s, sensor_id = %d\n",ic_model, gtp_sensor_id);
        ...
        if (touch_num)
        {
            for (i = 0; i < touch_num; i++)
            {
                ...
                //wisky-lxh@20140820
				if(gtp_touch_ic == TP_TYPE_GT9157 || gtp_touch_ic == TP_TYPE_GT911 
					|| gtp_touch_ic == TP_TYPE_GT9271 ) //wanglongteng@20141125 
				{
                	input_x = TPD_WARP_X(abs_x_max, input_x);
	                input_y = TPD_WARP_Y(abs_y_max, input_y);			
				}
				//end-wisky-lxh
				GTP_DEBUG("input_x:%d, input_y:%d\n", input_x, input_y);
