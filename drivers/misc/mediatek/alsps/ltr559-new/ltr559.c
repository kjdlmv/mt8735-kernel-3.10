...
static ssize_t ltr559_store_reg(struct device_driver *ddri, char *buf, size_t count)
{
	...
	#if 0
	if(2 == sscanf(buf, "%d %d ", &(u32)reg,&value))
	{ 
...
{
	int error;
	//yiwei@wisky 20150115
	#if defined(WISKY_4G016) || defined(WISKY_4G012) 
...
static int ltr559_init_device(void)
{
    ...
    error = ltr559_i2c_write_reg(LTR559_PS_N_PULSES, 0x5); 
	if(error<0)
    {
...
static int ltr559_get_als_value(struct ltr559_priv *obj, u16 als)
{
	int idx;
	int invalid = 0;
	APS_DBG("ltr559_get_als_value als  = %d\n",als); 
