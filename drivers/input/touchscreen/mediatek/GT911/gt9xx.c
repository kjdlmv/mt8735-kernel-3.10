...
static unsigned short force[] = {0, (0xBA>>1), I2C_CLIENT_END, I2C_CLIENT_END};
...
static s8 gtp_i2c_test(struct i2c_client *client)
{
    ...
    while (retry++ < 5)
    {
        ...
        GTP_ERROR("GTP_REG_HW_INFO ====%x-----: %08X", client->addr,hw_info);
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	GTP_ERROR("I2C communication ERROR!====%x\n",client->addr);
    ...
    GTP_INFO("create proc entry %s 8888888success", GT91XX_CONFIG_PROC_FILE);
 ...
 /*   gt91xx_config_proc = proc_create(GT91XX_CONFIG_PROC_FILE, 0666, NULL, &config_proc_ops);
	...
	GTP_INFO("create proc entry %s s666666666666uccess", GT91XX_CONFIG_PROC_FILE);
...
GTP_INFO("create proc entry %s s6667777777777777776uccess\n", GT91XX_CONFIG_PROC_FILE);
...
static int touch_event_handler(void *unused)
{
    ...
    do
    {
        ...
        if (touch_num)
        {
            ...
            #endif
                {
                    GTP_DEBUG("55555555=== (%d)(%d, %d)[%d] \n", id, input_x, input_y, input_w);
           //  tpd_down(input_x, input_y, input_w, id);
                //  	 tpd_down(1280-input_x, input_y, input_w, id); 
		tpd_down(input_y ,input_x , input_w, id);//good
                }
            }
        }
        else
