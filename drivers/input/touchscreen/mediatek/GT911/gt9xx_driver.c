...
#define GTP_INFO printk
#define GTP_ERROR printk

#define GTP_DEBUG printk
...
s32 gtp_read_version(struct i2c_client *client, u16 *version)
{
	...
	if (buf[5] == 0x00) {
		...
		if (buf[5] == 'S' || buf[5] == 's')
			chip_gt9xxs = 1;
		GTP_INFO("IC VERSION55==:%c%c%c%c_%02x%02x",
...
static void	tpd_gpio_output(int a,int val)
{
	if(val)
	{
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//tpd_gpio_as_int(GTP_INT_PORT);
 	...
 	mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, IRQF_TRIGGER_FALLING, tpd_eint_interrupt_handler, 0); 
...
#endif
		GTP_ERROR("I2C 888888888888==tpd_halt=%d   %d\n ",tpd_halt,tpd_eint_mode);
...
static void tpd_resume(struct device *h)
{
	...
	doze_status = DOZE_DISABLED;
#else
	GTP_DEBUG("resume 000000000\n");

	
	/* mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM); */
	enable_irq(touch_irq);
	GTP_DEBUG("resume 111111\n");
...
static struct i2c_board_info __initdata gt911_i2c_tpd={ I2C_BOARD_INFO("mtk-gt911", (0xBA))};

/* called when loaded into kernel */
static int __init tpd_driver_init(void)
{
	GTP_INFO("MediaTek gt91xx touch panel driver init\n");
	i2c_register_board_info(1, &gt911_i2c_tpd, 1);
	//tpd_get_dts_info();
