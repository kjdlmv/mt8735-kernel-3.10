...
int btn_key_x[3] = {130,360,590};
int btn_key_y[3] = {1380,1380,1380};
#define TPD_KEYS        {KEY_BACK,KEY_HOMEPAGE,KEY_MENU}
#define TPD_KEYS_DIM    {{130,1380,50,30},{360,1380,50,30},{590,1380,50,30}}
...
//#define  GPIO_CTP_RST_PIN 		84 //(GPIO2 | 0x80000000)

//#define GPIO_CTP_EINT_PIN          10 //(GPIO5 | 0x80000000)
...
int icn85xx_i2c_txdata(unsigned short addr, char *txdata, int length)
{
    ...
    //icn85xx_trace("tmp_buf[0]: 0x%x\n", tmp_buf[0]);
    //icn85xx_trace("tmp_buf[1]: 0x%d\n", tmp_buf[1]);
...
static void icn85xx_gesture_handler(void)
{
	...
	icn85xx_trace("++++++icn85xx_gesture_handler+++++++++++++++\n");
	...
	{	
		...
		gesture_id = buf[0];
		icn85xx_trace("[icn85xx_gesture_handler] buf[0]=0x%x -------\n",buf[0]);
		...
		if(gesture_status)
		{
			...
			printk("[%s] report guesture key", __func__);
...
static ssize_t icn85xx_gesture_enable_read_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
    char *ptr = page;    
    ...
    ptr += sprintf(ptr, "0x%02x", gtp_wakeup_flag);
    ptr += sprintf(ptr, "\n");
    *ppos += ptr - page;
    return (ptr - page);
}

static ssize_t icn85xx_gesture_enable_write_proc(struct file *filp, const char __user *buffer, size_t count, loff_t *off)
{
	  int i;
    S32 ret = 0;
    char char_gesen[3]={0};
    ...
    printk("print hex data gt9xx_enable=0x%x\n", (u8)enable); 
    gtp_wakeup_flag = (u8)enable;
	  return count; 
}

/* Node: ft63xx_gesture--->save the value for gesture*/
static ssize_t icn85xx_gesval_read_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
	  char *ptr = page;
	  if (*ppos)
    {
...
static ssize_t icn85xx_gesval_write_proc(struct file *filp, const char __user *buffer, size_t count, loff_t *off)
{
	  int i;
    S32 ret = 0;
    char char_gesval[3]={0};	
    unsigned long val;
    printk("%s,%d write count=%d\n",__func__, __LINE__, (int)count);
    if(count>3)
    {
    		count = 3;
    }
    ...
    {
    	  printk("%s,%d: copy from user fail\n",__func__, __LINE__);
        return -EFAULT;
    }
    ...
    for(i=0; i<3; i++)
    {
    	  printk("%s:char_gesval[i]=%x\n", __func__, char_gesval[i]);
    }
    
    ret = kstrtoul(char_gesval, 16, &val);
    if(ret<0)
    {
			  printk("%s kstrtoul error!!\n",__func__);
			  return -EINVAL;
    }
    
    printk("print hex data val=0x%x\n", (u8)val);    
    gesval = (u8)val;
    printk("print hex data gesval=0x%x\n", gesval);
          
    return count;
}


static const struct file_operations icn85xx_gesture_enable_proc_fops = { 
    .write = icn85xx_gesture_enable_write_proc,
    .read = icn85xx_gesture_enable_read_proc
};

static const struct file_operations gesval_proc_ops = {
    .owner = THIS_MODULE,
    .read = icn85xx_gesval_read_proc,
    .write = icn85xx_gesval_write_proc,
};
...
/*
	...
	for (i = 0, sum = 0; i < POINT_NUM * POINT_SIZE + 2; i++)
		sum += buf[i];
	
	if (sum != buf[POINT_NUM * POINT_SIZE + 2]) {
		printk("icn85xx checksum error!\n");
		icn85xx_ts_release();
		return;
	}	
*/	
		...
		if(button != 0)
		{
			...
			for(i = 0; i < TPD_KEY_COUNT; i++)
			{
				...
				if (button & (0x01 << i))
				{
					...
					tpd_button(btn_key_x[i],btn_key_y[i],1);
				}     
...
static int tpd_irq_registration(void)
{
	...
	if (node){
		...
		if (!touch_irq){
			pr_info("mtk_tpd Failed in irq_of_parse_and_map\n");
			return -EINVAL;
		}
		printk("mtk_tpd irq = %d\n",touch_irq);
	}else{
		pr_info("mtk_tpd Failed in of_find_compatible_node\n");
	}

	ret = request_irq(touch_irq, tpd_eint_interrupt_handler, EINTF_TRIGGER_FALLING, "TOUCH_PANEL-eint", NULL);
	if (ret < 0){
		printk(KERN_ERR"mtk_tpd Failed to request_irq. retval = %d\n", ret);
	}
	return ret;
}
...
static int  tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    else if(icn85xx_ts->ictype == ICN85XX_WITH_FLASH_87)
    {
	    ...
	    #if defined(CONFIG_PRIZE_HARDWARE_INFO)
			sprintf(current_tp_info.chip, "icn86xx_0x%02x\n  fw:0x%04x",icn85xx_ts->ictype,curVersion);
			sprintf(current_tp_info.more,"touchpanel");
			sprintf(current_tp_info.id,"0x%04x",client->addr);
		#endif
      //prize-lixuefeng-20150512-end
     icn85xx_ts->screen_max_x = TPD_RES_X;//tpd_dts_data.tpd_resolution[0];
     icn85xx_ts->screen_max_y = TPD_RES_Y;//tpd_dts_data.tpd_resolution[1];
...
//-----------------add TPD_SLIDE_WAKEUP-----------------------------
    ...
    icn85xx_gesture_enable_proc = proc_create(GESTURE_ENABLE_PROC_FILE, 0666, NULL, &icn85xx_gesture_enable_proc_fops);
  	
    if (icn85xx_gesture_enable_proc == NULL)
    {
        printk("create_proc_entry %s failed\n", GESTURE_ENABLE_PROC_FILE);
    }
	
	
	 icn85xx_gesval_proc = proc_create(ICN85XX_GESVAL_PROC_NAME, 0666, NULL, &gesval_proc_ops);
    if ( NULL == icn85xx_gesval_proc)
    {
        printk("create_proc_entry %s failed\n", ICN85XX_GESVAL_PROC_NAME);
    }
    else
    {
        printk("create proc entry %s success", ICN85XX_GESVAL_PROC_NAME);
    }
#endif
//------------------------------------------------------------------
      // set for int
...
//=======================================Gesture  CODE===============================
    ...
    icn85xx_trace("==icn85xx_ts_resume in==\n");
	//icn85xx_ts_reset();
    if((icn85xx_ts->ictype == ICN85XX_WITHOUT_FLASH_85) || (icn85xx_ts->ictype == ICN85XX_WITHOUT_FLASH_86)) 
    {
    ...
    icn85xx_trace("icn85xx_ts_resume zeni\n");
