...
static void icn85xx_report_value_A(void)
{
	...
	{  
		...
		if((button_last != 0) && (button == 0))
		{
			...
			tpd_up((1280 - tpd_keys_dim_local[i][1]) * 720 / 1280, (720 - tpd_keys_dim_local[i][0]) * 1280 / 720,0);
			button_last = button;
			return 1;       
		}
		if(button != 0)
		{			
			for(i = 0; i < TPD_KEY_COUNT; i++)
			{
				...
				if (button & (0x01 << i))
				{
					...
					tpd_down((1280- tpd_keys_dim_local[i][1]) * 720 / 1280, (720 -tpd_keys_dim_local[i][0]) * 1280 / 720, 0,0);
				}     
	...
	for(i = 0; i < icn85xx_ts->point_num; i++)
	{
		...
		// x=icn85xx_ts->point_info[i].u16PosX ;
		// y=icn85xx_ts->point_info[i].u16PosY;
		// x=(720-x)*1280/720;
		// y=(1280-y)*720/1280;

		// icn85xx_ts->point_info[i].u16PosX=x ;
		// icn85xx_ts->point_info[i].u16PosY=y;
...
//		if(fwVersion > curVersion)
		if(fwVersion != curVersion) //wisky-lxh@20151109,p70x,p80x compatible
		{
			retry = 5;
			while(retry > 0)
			{
